...
internal struct GeocodeResult: Codable {
    ...
    let placemarks: [ViettelPlacemark]
}
...
// Used internally for flattening and transforming geometry.coordinates
...
/**
 A concrete subclass of `Placemark` to represent entries in a `ViettelPlacemark` object’s `superiorPlacemarks` property. These entries are like top-level geocoding results, except that they lack location information and are flatter, with properties directly at the top level.
 */
@objc(MBViettelPlacemark)
open class ViettelPlacemark: Placemark {
    ...
    public required init(from decoder: Decoder) throws {
        ...
        poiType = try container.decodeIfPresent(Int.self, forKey: .poiType) ?? -1
    ...
    @objc open var poiType: Int
    
    @objc open var type: String?
    
    @objc open var placeType: [String]?
    
    @objc open var isSaved: Bool
}
